function [ output,info ] = color_transfer( s_index,t_index,s_list,t_list,img_size, subimg_size )
img_num = length(s_list);
n = log2((img_size(1)*img_size(2)) / subimg_size^2) + 1;
message = '';
for i = 1:img_num
    s_ave = mean(mean(s_list{s_index(i)}));
    t_ave = mean(mean(t_list{t_index(i)}));
    ...
    s_dev = [s_dev_r, s_dev_g, s_dev_b];
    t_dev = [t_dev_r, t_dev_g, t_dev_b];
%     q_r = t_dev_r / s_dev_r;
%     q_g = t_dev_g / s_dev_g;
%     q_b = t_dev_b / s_dev_b;
%     q = [q_r,q_g,q_b];
    for column = 1:subimg_size
        for row = 1:subimg_size
            for k = 1:3
                if s_dev(k) == 0 || t_dev(k) == 0
                    s_list{s_index(i)}(row,column,k) = t_ave(k);
                else
                    q = t_dev(k) / s_dev(k);
                    s_list{s_index(i)}(row,column,k) = (s_list{s_index(i)}(row,column,k) - s_ave(k)) * q + t_ave(k);
                end
                subimage(row,column,k)=s_list{s_index(i)}(row,column,k);
            end
        end
    end
